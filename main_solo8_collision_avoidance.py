...
def example_script(name_interface, clib_path):
    ...
    # Initialize viewer
    if VIEWER:
        viewer_coll = viewerClient(collision_threshold, urdf="/home/ada/git/tnoel/solopython/coll_avoidance_modules/urdf/solo8_simplified.urdf", modelPath="/home/ada/git/tnoel/solopython/coll_avoidance_modules/urdf")
    ...
    while ((not device.hardware.IsTimeout()) and (clock() < 200)):
        ...
        if VIEWER :
            viewer_coll.display(np.concatenate(([0,0,0,0,0,0,0],device.q_mes)), c_dist_legs, c_wPoints, tau_q)
